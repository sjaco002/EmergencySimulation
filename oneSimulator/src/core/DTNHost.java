...
public class DTNHost implements Comparable<DTNHost> {
	...
	public DTNHost(List<MessageListener> msgLs,
        ...
        logger = Logger.getLogger("DTNHost");
	...
	public void update(boolean simulateConnections) {
        ...
        if (name.startsWith("s") && timeOffset == 0.1){
            ...
            //emergency
            int secondsPlace = Double.valueOf(Math.round(timeOffset)).intValue() % 10;
            int tenthsPlace = Double.valueOf(Math.round(timeOffset*10)).intValue() % 10;
            if (tenthsPlace == 0){
                //make one emergency every second
                
                //0-49 flood //quarter of city
                //50-79 fire //small area
                //80-89 storm //most of city
                //90-94 riot //small area
                //radius of 1500 will cover most of the map
                String Etype = "crash";
                if (Integer.parseInt(name.substring(name.length() - 1)) < 5){
                    //flood, happens every
                    Etype = "flood";
                }
                else if(Integer.parseInt(name.substring(name.length() - 1)) < 8){
                    Etype = "fire";
                }
            ...
            //users
            //100 per node, so everyone updates every ten seconds
            //Each user id is the node_id appended with the last two digits (second and 10th of second)
            //e.g. p1234u01
